<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <i class="ph ph-plus-circle text-neon"></i>
    ...
    <div class="max-w-4xl mx-auto mb-6 mt-2">
        <div class="flex items-center space-x-4 relative z-10">
            ...
            <div>
                ...
                <p class="text-sm text-gray-400 font-medium">Buat promo diskon baru dan tentukan paket yang berlaku</p>
            </div>
        </div>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="md:col-span-2 relative group">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Promo Pelajar / Mahasiswa" class="w-full pl-11 py-3 border-gray-700/80 rounded-xl bg-dark/50 text-white focus:ring-neon focus:border-neon focus:bg-dark transition-all duration-300 shadow-inner">
                        </div>
                        <p class="mt-2 text-xs text-gray-500 flex items-center gap-1"><i class="ph ph-info"></i> Akan ditampilkan pada dropdown pendaftaran member.</p>
                    </div>
                    ...
                    <div class="relative group">
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input type="number" name="percentage" id="percentage" value="{{ old('percentage') }}" min="1" max="100" required class="w-full pl-11 pr-12 py-3 border-gray-700/80 rounded-xl bg-dark/50 text-white focus:ring-neon focus:border-neon focus:bg-dark transition-all duration-300 shadow-inner font-mono text-lg font-bold">
                        ...
                        @foreach($packages as $pkg)
                            <label class="relative flex items-start p-5 cursor-pointer rounded-2xl border-2 border-gray-800/60 bg-dark/30 hover:bg-dark/80 hover:border-neon/50 transition-all duration-300 group has-[:checked]:border-neon has-[:checked]:bg-neon/5 has-[:checked]:shadow-[0_0_15px_rgba(212,255,0,0.1)]">
                                <div class="flex items-center h-6 mt-0.5">
                                    <input type="checkbox" name="gym_packages[]" value="{{ $pkg->id }}" class="w-5 h-5 bg-dark border-gray-600 rounded text-neon focus:ring-neon focus:ring-2 focus:ring-offset-2 focus:ring-offset-dark transition-all" {{ in_array($pkg->id, old('gym_packages', [])) ? 'checked' : '' }}>
                                </div>
                                ...
                                <!-- Decorative checkmark that appears when checked -->
