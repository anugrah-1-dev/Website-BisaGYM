            ...
            <form action="{{ route('discounts.update', $discount) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $discount->name) }}" required placeholder="Contoh: Promo Pelajar / Mahasiswa" class="w-full border-gray-700 rounded-lg bg-dark text-white focus:ring-neon focus:border-neon text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="percentage" id="percentage" value="{{ old('percentage', $discount->percentage) }}" min="1" max="100" required class="w-full border-gray-700 rounded-lg bg-dark text-white focus:ring-neon focus:border-neon text-sm pr-10">
                        ...
                        @foreach($packages as $pkg)
                            <label class="relative flex items-start p-4 cursor-pointer rounded-xl border border-gray-700 bg-dark hover:border-neon/50 transition-colors group">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" name="gym_packages[]" value="{{ $pkg->id }}" class="w-5 h-5 bg-dark border-gray-600 rounded text-neon focus:ring-neon focus:ring-2 focus:ring-offset-2 focus:ring-offset-dark" {{ in_array($pkg->id, old('gym_packages', $selectedPackages)) ? 'checked' : '' }}>
                                </div>
